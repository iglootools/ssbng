...
SSB is an rsync-based backup tool. The execution flow is:

```
CLI (cli.py) → Runner (runner.py) → Checks (checks.py) + Rsync (rsync.py) + Btrfs (btrfs.py)
                                         ↓                    ↓                    ↓
...
1. `check_all_syncs()` — verifies volumes are reachable and marker files exist (`.ssb-vol`, `.ssb-src`, `.ssb-dst`)
...
Search order: explicit path → `$XDG_CONFIG_HOME/ssb/config.yaml` → `/etc/ssb/config.yaml`. Raises `ConfigError` on validation failure.
