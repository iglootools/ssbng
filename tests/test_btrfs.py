...
class TestCreateSnapshotLocal:
    ...
    @patch("ssb.btrfs.datetime")
    def test_success(self, mock_dt: MagicMock, mock_run: MagicMock) -> None:
        mock_iso = MagicMock()
        mock_iso.replace.return_value = "2024-01-15T12:00:00.000Z"
        mock_dt.now.return_value.isoformat.return_value = mock_iso
    ...
    @patch("ssb.btrfs.subprocess.run")
    @patch("ssb.btrfs.datetime")
    def test_failure(self, mock_dt: MagicMock, mock_run: MagicMock) -> None:
        mock_iso = MagicMock()
        mock_iso.replace.return_value = "2024-01-15T12:00:00.000Z"
        mock_dt.now.return_value.isoformat.return_value = mock_iso
